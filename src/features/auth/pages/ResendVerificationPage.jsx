...
export default function ResendVerificationPage() {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-[var(--border)] bg-white p-8 shadow-[0_18px_40px_-26px_rgba(15,23,42,0.45)]">
      <div className="mb-7">
        <h1 className="text-3xl font-bold text-[var(--text)]">Reenviar verificación</h1>
        <p className="mt-2 text-sm text-[var(--muted)]">
          Te enviaremos un nuevo correo para activar tu cuenta.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          {isLoading ? 'Procesando...' : 'Reenviar correo'}
        </button>
      </form>

      <div className="mt-5 flex items-center justify-between text-sm">
        <Link to="/verify-email" className="font-semibold text-[var(--primary)] transition hover:underline">
          Tengo token para verificar
        </Link>
